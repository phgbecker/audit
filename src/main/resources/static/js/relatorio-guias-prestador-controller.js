$(document).ready(function() {
    ...
    $('#form-relatorio-guias-prestador').on('submit', function(e) {
        e.preventDefault();
        obterRegistrosExibirRelatorio();
    });
    ...
    function obterRegistrosExibirRelatorio() {
        ...
        $.ajax({
            url: '/rest/' + cnpj + '/relatorios/prestador?dataInicial=' + $('#filtro-data-inicial').val() + '&dataFinal=' + $('#filtro-data-final').val(),
            type: 'GET',
            beforeSend: function() {
                $('#form-relatorio-guias-prestador .alertas').empty().append('<div class="spinner-border spinner-border-sm" role="status"></div> Aguarde...');
            },
            complete: function() {
            },
            success: function(data, textStatus) {
                $('#form-relatorio-guias-prestador .alertas').empty();
            ...
            error: function(xhr,er) {
                switch (xhr.status) {
                    case 401:
                        ...
                        break;
                    default:
                        $('#form-relatorio-guias-prestador .alertas').empty().append('<span class="text-danger">Não foi possível realizar a operação</span>');
                }
