$(document).ready(function() {
    ...
    $('#form-relatorio-guias-idade').on('submit', function(e) {
        e.preventDefault();
        obterRegistrosExibirRelatorio();
    });
    ...
    function obterRegistrosExibirRelatorio() {
        ...
        $.ajax({
            url: '/rest/' + cnpj + '/relatorios/idade?dataInicial=' + $('#filtro-data-inicial').val() + '&dataFinal=' + $('#filtro-data-final').val(),
            type: 'GET',
            beforeSend: function() {
                $('#form-relatorio-guias-idade .alertas').empty().append('<div class="spinner-border spinner-border-sm" role="status"></div> Aguarde...');
            },
            complete: function() {
            },
            success: function(data, textStatus) {
                $('#form-relatorio-guias-idade .alertas').empty();
            ...
            error: function(xhr,er) {
                switch (xhr.status) {
                    case 401:
                        ...
                        break;
                    default:
                        $('#form-relatorio-guias-idade .alertas').empty().append('<span class="text-danger">Não foi possível realizar a operação</span>');
                }
